...
namespace CoreWebAPI.Common.Redis
{
    public class RedisBasketRepository : IRedisBasketRepository
    {
        ...
        public async Task<long> RemoveBegin(string key)
        {
            ...
            if (!redisResult.IsNull)
                return await _database.KeyDeleteAsync((RedisKey[])redisResult);  //删除一组key
            return 0;
        }

        public async Task<bool> Set(string key, object value, double expirySecond = 3600)
        {
            if (value != null)
                //序列化，将object值生成RedisValue
               return await _database.StringSetAsync(key, JsonHelper.Serialize(value), TimeSpan.FromSeconds(expirySecond));
            return false;
        }
        ...
        public async Task Set(string key, object value, TimeSpan cacheTime)
        {
            if (value != null)
                //序列化，将object值生成RedisValue
               await _database.StringSetAsync(key, JsonHelper.Serialize(value), cacheTime);
        }
