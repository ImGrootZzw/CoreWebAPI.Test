...
namespace CoreWebAPI.Common.HttpRestSharp
{
    /// <summary>
    ...
    public static class HttpHelper
    {
        ...
        /// <returns></returns>
        public static T GetApi<T>(string baseUrl, string url, string param = "")
        {
            var client = new RestSharpClient(baseUrl);

            var request = client.Execute(string.IsNullOrEmpty(param)
                ? new RestRequest(url, Method.GET)
                : new RestRequest($"{url}?{param}", Method.GET));
